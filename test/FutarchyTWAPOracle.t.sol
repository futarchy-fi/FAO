...
import {MockAlgebraPoolTWAP, GasGriefingAlgebraPool} from
    "./mocks/MockAlgebraPoolTWAP.sol";
...
contract FutarchyTWAPOracleTest is Test {
    ...
    function setUp() public {
        oracle = new FutarchyTWAPOracle(
            dao, binderAddr, TRADING_PERIOD, TWAP_WINDOW, THRESHOLD
        );
    ...
    function testConstructorRevertsZeroDAO() public {
        ...
        new FutarchyTWAPOracle(
            address(0), binderAddr, TRADING_PERIOD, TWAP_WINDOW, THRESHOLD
        );
    }
    ...
    function testConstructorRevertsZeroBinder() public {
        vm.expectRevert(FutarchyTWAPOracle.ZeroAddress.selector);
        new FutarchyTWAPOracle(
            dao, address(0), TRADING_PERIOD, TWAP_WINDOW, THRESHOLD
        );
    }
    ...
    function testConstructorRevertsInvalidConfig() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                FutarchyTWAPOracle.InvalidConfig.selector, 100, 200
            )
        );
    ...
    function testBindRevertsIfAlreadyBound() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                FutarchyTWAPOracle.AlreadyBound.selector, proposal
            )
        );
        oracle.bind(
            proposal,
    ...
    function testResolveRevertsIfNotBound() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                FutarchyTWAPOracle.NotBound.selector, proposal
            )
        );
    ...
    function testResolveRevertsBeforeTradingEnds() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                FutarchyTWAPOracle.TradingNotEnded.selector,
                proposal,
                deadline,
                block.timestamp
            )
    ...
    function testResolveRevertsIfAlreadyResolved() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                FutarchyTWAPOracle.AlreadyResolved.selector, proposal
            )
        );
        oracle.resolve(proposal);
    }
    ...
    function testResolveNoAtThresholdBoundary() public {
        ...
        FutarchyTWAPOracle oracleT = new FutarchyTWAPOracle(
            dao, binderAddr, TRADING_PERIOD, TWAP_WINDOW, 10
        );
    ...
    function testResolveWithNegativeTicks() public {
        ...
        yesPool.setTickCumulatives(
            0, -50 * int56(int32(TWAP_WINDOW))
        );
        noPool.setTickCumulatives(
            0, -100 * int56(int32(TWAP_WINDOW))
        );
    ...
    function testResolveEmitsEvent() public {
        ...
        vm.expectEmit(true, false, false, true);
        emit FutarchyTWAPOracle.ProposalResolved(
            proposal, true, 100, 50
        );
        oracle.resolve(proposal);
    }
    ...
    function testResolveInvertedTokenOrdering() public {
        ...
        MockAlgebraPoolTWAP invertedYesPool =
            new MockAlgebraPoolTWAP(currencyLow, companyHigh);
        MockAlgebraPoolTWAP invertedNoPool =
            new MockAlgebraPoolTWAP(currencyLow, companyHigh);
        ...
        invertedYesPool.setTickCumulatives(
            0, -100 * int56(int32(TWAP_WINDOW))
        );
        invertedNoPool.setTickCumulatives(
            0, -50 * int56(int32(TWAP_WINDOW))
        );
    ...
    function testResolveMixedOrdering() public {
        ...
        MockAlgebraPoolTWAP normalYesPool =
            new MockAlgebraPoolTWAP(yesCompany, yesCurrency);
        MockAlgebraPoolTWAP invertedNoPool2 =
            new MockAlgebraPoolTWAP(noCurrencyLow, noCompanyHigh);
        ...
        normalYesPool.setTickCumulatives(
            0, 80 * int56(int32(TWAP_WINDOW))
        );
        invertedNoPool2.setTickCumulatives(
            0, -60 * int56(int32(TWAP_WINDOW))
        );
    ...
    function testResolveRevertsInsufficientGasOnYesPool() public {
        ...
        GasGriefingAlgebraPool gasPool =
            new GasGriefingAlgebraPool(yesCompany, yesCurrency);
    ...
    function testGetDecisionUnboundProposal() public view {
        (bool resolved, bool accepted) =
            oracle.getDecision(address(0xDEAD));
        assertFalse(resolved);
        assertFalse(accepted);
    }
    ...
    function testSetConfigRevertsInvalidWindow() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                FutarchyTWAPOracle.InvalidConfig.selector, 1 days, 2 days
            )
        );
