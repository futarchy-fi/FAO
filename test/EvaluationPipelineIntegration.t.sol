...
contract EvaluationPipelineIntegrationTest is Test {
    ...
    /// tokens.
    function _makeProposal() internal returns (MockFutarchyProposalLike) {
    ...
    function _createEvaluatingProposal() internal returns (uint256 proposalId) {
        uint256 m = 1e18;
    ...
    function testFullLifecycleAccepted() public {
        ...
        assertEq(uint256(p.state), uint256(FutarchyArbitration.ProposalState.EVALUATING));
    ...
    function testResolveRevertsIfFutarchyNotYetResolved() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                EvaluationPipeline.FutarchyNotResolved.selector, address(futarchyProp)
            )
        );
