...
import {MockFutarchyArbitrationLike} from
    "./mocks/MockFutarchyArbitrationLike.sol";
...
contract EvaluationPipelineTest is Test {
    ...
    function _setupProposal(uint256 futarchyId)
        internal
        returns (MockFutarchyProposalLike prop)
    {
    ...
    function testStartEvaluationRevertsIfNoActiveEvaluation() public {
        ...
        vm.expectRevert(
            EvaluationPipeline.NoActiveEvaluation.selector
        );
        pipeline.startEvaluation(
            1, "test", "cat", "en", 1e18, uint32(block.timestamp)
        );
    }
    ...
    function testStartEvaluationRevertsIfWrongProposalId() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                EvaluationPipeline.WrongProposalId.selector, 7, 99
            )
        );
        pipeline.startEvaluation(
            99, "test", "cat", "en", 1e18, uint32(block.timestamp)
        );
    }
    ...
    function testStartEvaluationSucceeds() public {
        ...
        pipeline.startEvaluation(
            5, "test market", "cat", "en", 1e18, uint32(block.timestamp)
        );

        assertEq(
            pipeline.futarchyProposalOf(5), address(prop)
        );
    ...
    function testStartEvaluationBindsCorrectPools() public {
        ...
        pipeline.startEvaluation(
            5, "test", "cat", "en", 1e18, uint32(block.timestamp)
        );
    ...
    function testStartEvaluationRevertsIfAlreadyStarted() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                EvaluationPipeline.EvaluationAlreadyStarted.selector, 5
            )
        );
        pipeline.startEvaluation(
            5, "test", "cat", "en", 1e18, uint32(block.timestamp)
    ...
    function testStartEvaluationEmitsEvent() public {
        ...
        emit EvaluationPipeline.EvaluationMarketCreated(
            5, 42, address(prop)
        );
    ...
    function testResolveRevertsIfWrongProposalId() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                EvaluationPipeline.WrongProposalId.selector, 7, 8
            )
        );
        pipeline.resolve(8);
    }
    ...
    function testResolveRevertsIfEvaluationNotStarted() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                EvaluationPipeline.EvaluationNotStarted.selector, 7
            )
        );
        pipeline.resolve(7);
    }
    ...
    function testResolveRevertsIfFutarchyNotResolved() public {
        ...
        MockFutarchyProposalLike prop = _setupProposal(42);
        pipeline.startEvaluation(
            7, "test", "cat", "en", 1e18, uint32(block.timestamp)
        );
    ...
    function testResolveEmitsEvent() public {
        ...
        emit EvaluationPipeline.EvaluationResolved(
            7, address(prop), true
        );
