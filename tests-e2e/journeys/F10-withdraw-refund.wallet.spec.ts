/**
 * F10-withdraw-refund scaffold — see audit/rubrics/topic-2-interface-testing.md.
 *
 * After settlement, the losing bonder withdraws their refund. Persona: NO bonder on a YES-settled proposal. Asserts withdrawable[i] decremented + EOA WETH delta.
 *
...
test.fixme('F10-withdraw-refund happy path', async ({ page }) => {
  await page.goto('/');
  // TODO: implement happy path per the description above.
});

test.fixme('F10-withdraw-refund — wallet rejection', async ({ page }) => {
...
test.fixme('F10-withdraw-refund — wrong chain', async ({ page }) => {
...
test.fixme('F10-withdraw-refund — RPC 5xx during dispatch', async ({ page }) => {
