...
struct MetaTransaction {
    ...
    // We require a salt so that the struct can always be unique and we can use its hash as a unique
    // identifier.
    uint256 salt;
}

/// @dev    Structure used for the function `initialize` of the Space contract because of solidity's
/// stack constraints. For more information, see `ISpaceActions.sol`.
...
/// @dev    Structure used for the function `updateSettings` of the Space contract because of
/// solidity's stack constraints. For more information, see `ISpaceOwnerActions.sol`.
