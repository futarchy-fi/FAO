...
contract FAOFutarchyFactory {
    ...
    function computeQuestionId(string memory marketName, string memory description, uint256 proposalIndex)
        public
        view
        returns (bytes32)
    {
        ...
        return keccak256(
            abi.encodePacked(contentHash, address(this), proposalIndex, block.prevrandao)
        );
    }
    ...
    function createProposal(CreateProposalParams memory params) external returns (address) {
        ...
        bytes32 questionId_ = computeQuestionId(params.marketName, params.description, proposalIndex);
        ...
        (address[] memory wrapped1155, bytes[] memory tokenData) =
            _deployERC20Positions(params.collateralToken1, params.collateralToken2, conditionId_, tokenNames);

        FAOFutarchyProposal.FAOFutarchyProposalParams memory proposalParams = FAOFutarchyProposal
            .FAOFutarchyProposalParams({
        ...
        instance.initialize(params.marketName, params.description, outcomes, proposalParams, IFAOFutarchyOracle(oracle));

        proposals.push(address(instance));

        emit NewProposal(proposalIndex, address(instance), conditionId_, questionId_, bytes32(block.prevrandao));
    ...
    function _deployERC20Positions(
        ...
        for (uint256 j = 0; j < 4; j++) {
            ...
            bytes32 collectionId = conditionalTokens.getCollectionId(bytes32(0), conditionId_, indexSet);
            ...
            address wrapper = wrapped1155Factory.requireWrapped1155(address(conditionalTokens), tokenId, tokenData);
