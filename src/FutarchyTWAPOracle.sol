...
contract FutarchyTWAPOracle {
    ...
    error TradingNotEnded(
        address proposal, uint256 deadline, uint256 currentTime
    );
    ...
    event ConfigUpdated(
        uint32 tradingPeriod, uint32 twapWindow, int24 thresholdTicks
    );
    ...
    function bind(
        ...
        emit ProposalBound(
            proposal, yesPool, noPool, uint48(block.timestamp)
        );
    }
        ...
        returns (bool accepted)
    {
        ...
        (int56 yesAvgTick, bool yesFailed) =
            _computeTWAP(p.yesPool, p.yesBase, deadline);
        (int56 noAvgTick, bool noFailed) =
            _computeTWAP(p.noPool, p.noBase, deadline);
        ...
        if (yesFailed || noFailed) {
            ...
            accepted =
                yesAvgTick > noAvgTick + int56(int24(thresholdTicks));
        }
    ...
    function _computeTWAP(
        ...
        address baseToken,
        uint256 deadline
    ) internal view returns (int56 economicAvgTick, bool failed) {
        ...
        try IAlgebraPoolTWAP(pool).getTimepoints(secondsAgos) returns (
            ...
            // orientation.
            int56 sign = IAlgebraPoolTWAP(pool).token0() == baseToken
                ? int56(1)
                : int56(-1);
            economicAvgTick =
                (sign * tickDelta) / int56(int32(window));
        } catch {
