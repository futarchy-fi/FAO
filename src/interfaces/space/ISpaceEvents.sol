...
interface ISpaceEvents {
    ...
    event ProposalCreated(
        uint256 proposalId, address author, Proposal proposal, string metadataUri, bytes payload
    );
    ...
    event VoteCastWithMetadata(
        uint256 proposalId, address voter, Choice choice, uint256 votingPower, string metadataUri
    );
    ...
    event VotingStrategiesAdded(
        Strategy[] newVotingStrategies, string[] newVotingStrategyMetadataURIs
    );
    ...
    /// @param newProposalValidationStrategyMetadataURI The metadata URI for the proposal validation
    /// strategy.
    event ProposalValidationStrategyUpdated(
        Strategy newProposalValidationStrategy, string newProposalValidationStrategyMetadataURI
    );
