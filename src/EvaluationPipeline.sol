...
interface IFutarchyArbitrationForPipeline {
    function activeEvaluationProposalId()
        external
        view
        returns (uint256);
...
interface IFutarchyProposalOutcomes {
    function wrappedOutcome(uint256 index)
        external
        view
        returns (address token, bytes memory data);
}
...
contract EvaluationPipeline is IFutarchyArbitrationEvaluator {
    ...
    mapping(uint256 proposalId => address futarchyProposal)
        public futarchyProposalOf;
    ...
    {
        uint256 active = IFutarchyArbitrationForPipeline(
            arbitrationContract
        ...
        (bool resolved, bool result) =
            twapOracle.getDecision(futarchyProposal);
        if (!resolved) revert FutarchyNotResolved(futarchyProposal);

        accepted = result;
        IFutarchyArbitrationForPipeline(arbitrationContract)
            .resolveActiveEvaluation(accepted);

        emit EvaluationResolved(
            proposalId, futarchyProposal, accepted
        );
    }
    ...
    function startEvaluation(
        ...
        emit EvaluationMarketCreated(
            proposalId, futarchyProposalId, futarchyProposal
        );
    }
    ...
    function _bindToTWAPOracle(address futarchyProposal) internal {
        IFutarchyProposalOutcomes fp =
            IFutarchyProposalOutcomes(futarchyProposal);
        ...
        address yesPool =
            algebraFactory.poolByPair(yesCompany, yesCurrency);
        address noPool =
            algebraFactory.poolByPair(noCompany, noCurrency);
