---
canonical: audit/rubrics/topic-6-llm-wiki.md@15279877e01f6dea50b96bf056302060e3ab6214
...
A rebuild starts from the outline and the Topic-6 rubric. The outline fixes the tree and build order; the rubric scores source traceability, cross-link graph health, convergence, abstraction quality, abstention, and freshness. `audit/wiki/_OUTLINE.md:1-46`, `audit/rubrics/topic-6-llm-wiki.md:34-185`
...
Every concrete implementation claim should cite a file line range or URL immediately near the claim. The rubric grades citation existence, citation resolution, and whether the cited region actually supports the claim; line ranges alone are not enough if they point at the wrong symbol. `audit/rubrics/topic-6-llm-wiki.md:36-56`

Each full page ends with provenance naming the builder, the source commit read, and the build pass. This repository is currently pinned in this pass to commit `15279877e01f6dea50b96bf056302060e3ab6214`, which is the value returned by `git rev-parse HEAD` during the build. `audit/wiki/_OUTLINE.md:48-57`
...
- If Topic-6 weights change, this page's maintenance contract should be re-read against the new rubric rather than copied forward. `audit/rubrics/topic-6-llm-wiki.md:17-27`
