...
### DEPR-10 — hand-maintained site registry ABI literal

| | |
|---|---|
| **Status** | SUPERSEDED |
| **Source** | `site-testnet/shared.js::REGISTRY_ABI` and matching ad hoc registry ABI snippets. |
| **Replaced by** | `site-testnet/abis/FutarchyRegistry.json` generated by `scripts/sync-abis.sh`. |
| **Reason** | Handwritten ABI fragments can silently omit or mismatch contract functions even while addresses stay synced. Generated ABI JSON ties the site read path back to the compiled Solidity interface. |
| **Rule** | Site code SHOULD load generated ABI JSON via `window.loadFaoAbi(contractName)` instead of adding new hardcoded ABI literals for FAO contracts. |
